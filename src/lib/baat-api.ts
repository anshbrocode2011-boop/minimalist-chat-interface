...
function headers(token?: string) { return { "Content-Type": "application/json", ...(token ? { Authorization: `Bearer ${token}` } : {}) }; }
async function request<T>(path: string, options: RequestInit = {}, token?: string): Promise<T> {
  const response = await fetch(`${API_BASE_URL}${path}`, { ...options, headers: { ...headers(token), ...(options.headers ?? {}) } });
  if (!response.ok) { let message = "Request failed"; try { const payload = await response.json(); if (typeof payload?.error === "string") message = payload.error; } catch { /* non-JSON error */ } throw new Error(message); }
...
export const createChat = (chatId: string, token: string) => request<{ id: string }>("/api/chats", { method: "POST", body: JSON.stringify({ chatId: chatId.toUpperCase() }) }, token);
...
export const sendMessage = (chatId: string, body: string, token: string) => request<ChatMessage>(`/api/chats/${chatId}/messages`, { method: "POST", body: JSON.stringify({ body }) }, token);
export function readStoredSession(): { token: string; user: BaatUser } | null { if (typeof window === "undefined") return null; const token = localStorage.getItem("baat-token"); const rawUser = localStorage.getItem("baat-user"); if (!token || !rawUser) return null; try { return { token, user: JSON.parse(rawUser) as BaatUser }; } catch { clearSession(); return null; } }
