...
const frontendDir = path.resolve(__dirname, "../../.output/public");

type Identity = { id: string; chatId: string };
type AuthRequest = Request & { user?: Identity };

app.use(helmet({ contentSecurityPolicy: false }));
const origins = (process.env.CLIENT_ORIGIN ?? "").split(",").map((value) => value.trim()).filter(Boolean);
...
app.get("*", async (req, res, next) => { if (req.path.startsWith("/api/") || req.path === "/health") return next(); try { res.sendFile(path.join(frontendDir, "index.html")); } catch { res.status(404).send("Not found"); } });

async function start() { if (process.env.DATABASE_URL) { const schema = await fs.readFile(path.resolve(__dirname, "../schema.sql"), "utf8"); await pool.query(schema); } server.listen(port, "0.0.0.0", () => console.log(`Baat listening on port ${port}`)); }
