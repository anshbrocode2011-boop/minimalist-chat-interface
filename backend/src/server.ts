...
app.post("/api/auth/login", async (req, res) => {
  ...
  if (!result.rowCount || !(await argon2.verify(result.rows[0].password_hash, parsed.data.password))) {
...
app.post("/api/chats/:id/messages", auth, async (req: AuthRequest, res) => {
  ...
  const message = await pool.query(
    ...
    [req.params.id, req.user!.id, body.data],
  );
  ...
  const event = {
    ...
    message: { id: message.rows[0].id, body: message.rows[0].body, time: message.rows[0].created_at, mine: true },
  };

  member.rows.forEach((row) => sendTo(row.user_id, event));
  return res.status(201).json(event.message);
});
...
async function start() {
  ...
  server.listen(port, "0.0.0.0", () => {
    console.log(`Baat listening on port ${port}`);
  });
}
